...
def create_persona():
    data = request.json
    nueva_persona = Persona(**data)  # Asume que data tiene los campos correctos
...
def get_personas():
    ...
    return jsonify([{'id': p.id, 'nombre': p.nombre, 'email': p.email} for p in personas])

@personas_bp.route('/personas/<int:id>', methods=['GET'])
def get_persona(id):
    persona = Persona.query.get(id)
    if persona:
        return jsonify({'id': persona.id, 'nombre': persona.nombre, 'email': persona.email})
